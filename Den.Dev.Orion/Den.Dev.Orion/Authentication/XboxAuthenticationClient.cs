...
namespace Den.Dev.Orion.Authentication
{
    ...
    public class XboxAuthenticationClient
    {
        ...
        private ECDCertificatePoPCryptoProvider popCryptoProvider = new();
        ...
        /// <summary>
        /// Loads an existing proof key locally to make sure that the device tokens
        /// generated by the library match the requirements across sessions.
        /// </summary>
        /// <param name="path">Path to the proof key JSON file.</param>
        /// <returns>If successful, returns true. Otherwise, throws an exception.</returns>
        public bool LoadProofKey(string path)
        {
            if (!File.Exists(path))
            {
                throw new FileNotFoundException($"Proof key file {path} does not exist.");
            }

            try
            {
                ProofKey? proofKey = JsonSerializer.Deserialize<ProofKey>(File.ReadAllText(path)) ?? throw new FileLoadException($"Could not deserialize the proof key from the specified JSON file ({path}).");
                this.popCryptoProvider.ProofKey = proofKey;
                return true;
            }
            catch (Exception ex) when (ex is IOException || ex is JsonException)
            {
                throw new FileLoadException($"An error occurred while loading the proof key from the file {path}.", ex);
            }
        }

        /// <summary>
        /// Saves the current proof key to a specified path in JSON format.
        /// </summary>
        /// <param name="path">Path to save the proof key JSON file.</param>
        /// <returns>If successful, returns true. Otherwise, throws an exception.</returns>
        public bool SaveProofKey(string path)
        {
            if (this.popCryptoProvider.ProofKey == null)
            {
                throw new InvalidOperationException("The current proof key is null and cannot be saved.");
            }

            try
            {
                File.WriteAllText(path, JsonSerializer.Serialize(this.popCryptoProvider.ProofKey));
                return true;
            }
            catch (Exception ex)
            {
                throw new IOException($"An error occurred while saving the proof key to the file {path}.", ex);
            }
        }
